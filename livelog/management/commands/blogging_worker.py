...
from livelog.pub import JsonCustomSerializer
import const
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while True:
            now_time = time.time()
            Group(const.GROUP_NAME).send({"text": json.dumps(now_time,cls=JsonCustomSerializer)})
            logger.debug('The time now is %s' % now_time)
            time.sleep(5)